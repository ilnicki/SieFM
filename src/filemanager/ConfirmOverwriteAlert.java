...
/**
 * Класс для выдачи сообщения типа "Файл ... существует. Перезаписать?" с
 * вариантами ответа Да / Нет / Да для всех / Нет для всех / Отмена.
 */
public class ConfirmOverwriteAlert extends Alert implements CommandListener, Runnable
{
    ...
    /**
     * Конструирует класс и создаёт поток, ожидающий выбора команды. А lstBuffer
     * юзает al.t.join() для того, чтобы подождать.
     *
     * @param filename
     * @param nextDsp
     */
    public ConfirmOverwriteAlert(String filename, Displayable nextDsp)
    {
