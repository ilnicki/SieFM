...
class CopyMoveThread implements Runnable
{
    ...
    public void run()
    {
        ...
        {
            mustMove = Buffer.move.get(Buffer.buf.size() - 1) == 1; // true, если надо перемещать!
            sourceFileFullName = noLastSlash(
                    (String) Buffer.buf.elementAt(Buffer.buf.size() - 1)); // посл файл из буфера
            sourceOnlyName = sourceFileFullName
                    .substring(sourceFileFullName.lastIndexOf('/') + 1);
            ...
            {
                ConfirmOverwriteAlert al = new ConfirmOverwriteAlert(sourceOnlyName, Main.FileSelect);
        ...
        Main.dsp.setCurrent(new MessageAlert(Buffer.errors));
    }

    /**
     * Обрезание последнего слэша в строке s.
     */
...
class AfterburnThread implements Runnable
{
    ...
    public void run()
    {
        ...
        Buffer.clear();
        Main.dsp.setCurrent(new MessageAlert(Buffer.zz.error));
        Buffer.zz = null;
    }
}
